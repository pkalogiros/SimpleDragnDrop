	...
	**/
	parent.dragNDrop = function( body, overlay_id, callback, method, _clss ) {
		...
		this.destroy = function() {
			body.removeEventListener( "dragenter", events.onDragEnter, false );
			overlay.removeEventListener( "dragleave", events.onDragLeave, false );
			overlay.removeEventListener( "dragover", events.silencer, false);
			overlay.removeEventListener( "drop", events.onDrop, false);
			
			if( typeof overlay_id === 'string' )
				body.removeChild( overlay );
			
			body = overlay = overlay_id = events = callback = null;
			return false;
		};
